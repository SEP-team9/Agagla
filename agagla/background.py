...
class Star:
    ...
    def randomize(self, first_render):
        ...
        if first_render: self.brightness = randint(0, 255)
        else: self.brightness = 0
        self.dimming = self.brightness >= 255
        self.step_time = randint(25, 100) * 1_000_000
        self.speed = 0.10
        self.velocity = pygame.Vector2(self.rand_float(-self.speed, self.speed), self.rand_float(-self.speed, self.speed))
    ...
    def draw(self, screen):
        def draw_point(relative_x, relative_y):
            ...
            if our_x < 0 or our_y < 0 or our_x > WINDOW_WIDTH or our_y > WINDOW_HEIGHT: return
    ...
    def render(self):
        for i in range(0, NUM_STARS):
            ...
            if current is None or current.is_dead():
                current = Star(pygame.Vector2(randint(0, WINDOW_WIDTH), randint(0, WINDOW_HEIGHT)))
